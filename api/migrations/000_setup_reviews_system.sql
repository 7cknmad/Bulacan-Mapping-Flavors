-- Split statements to avoid DELIMITER issues
...
UPDATE dishes SET 
  ...
  avg_rating = COALESCE((SELECT SUM(rating * weight) / SUM(weight) FROM ratings WHERE rateable_type = 'dish' AND rateable_id = NEW.rateable_id), 0)
